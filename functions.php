...
function add_property_types_to_query( $query ) {
    if ( is_home() && $query->is_main_query() )
        $query->set( 'post_type', array( 'post', 'property' ) );
    return $query;
}
add_action( 'pre_get_posts', 'add_property_types_to_query' );

function property_js() {

    // add font
    wp_enqueue_style('hk-grotesk', '//fontlibrary.org/face/hk-grotesk');
    wp_enqueue_style('Montserrat', '//fonts.googleapis.com/css?family=Montserrat');
    wp_enqueue_style('Roboto', '//fonts.googleapis.com/css?family=Roboto:300,400,500,700|Google+Sans');
    wp_enqueue_style('Lato', '//fonts.googleapis.com/css?family=Lato:regular,700%7CLato:regular,400%7CLato:regular,700%7CDancing+Script');

    // custom css
    wp_enqueue_style('propertydetailcss', get_template_directory_uri() . '/css/propertydetail.css');
    wp_enqueue_script( 'custom', get_template_directory_uri() . '/js/custom.js', [], '', true);

    // slider plugin for header gallery
    wp_enqueue_style('slidercss', get_template_directory_uri() . '/css/lightslider.min.css');
    wp_enqueue_script( 'slider', get_template_directory_uri() . '/js/lightslider.min.js', [], '', true);

    // plugin for add event
    wp_enqueue_script('act', '//addevent.com/libs/atc/1.6.1/atc.min.js', [], '', true);
}
add_action('wp_enqueue_scripts','property_js');
